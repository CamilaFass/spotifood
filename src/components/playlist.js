...
function Playlist(props) {
  ...
  return (
    <div
      className={displayBy === "grid" ? "playlist" : "playlistrow"}
    ...
    >
      ...
      <div className={displayBy === "grid" ? "container" : "containerrow"}>
        ...
        <h1 className={displayBy === "grid" ? "title" : "titlerow"}>
          {data.name}
        </h1>
        <p className={displayBy === "grid" ? "desc" : "descrow"}>
          {data.description}
        </p>
        {displayBy === "list" && (
          <p className="footer">
            Created by {data.owner.display_name} {data.tracks.total} songs
          </p>
        )}
      </div>
    </div>
